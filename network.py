...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Lambda, BatchNormalization, Activation, Dropout, LeakyReLU
from tensorflow.keras.regularizers import l2


class SiameseNN(object):
  def __init__(self, config, seed, img_width, img_height, img_cells, loss, metrics, optimizer, dropout_rate):
        self.config = config
        K.clear_session()
        self.load_file = None
        self.seed = seed
        ...
        prediction = Dense(1, activation=self.config["final activation"], bias_initializer=self.initialize_bias)(L1_siamese_dist)
  ...
  def _get_architecture(self, input_shape):
        ...
        model = Sequential()
        model.add(
            Conv2D(filters=self.config["Layers_A"],
                   kernel_size=(self.config["Kernel_A"], self.config["Kernel_A"]),
        ...
        model.add(
            Conv2D(filters=self.config["Layers_B"],
                   kernel_size=(self.config["Kernel_B"], self.config["Kernel_B"]),
                   kernel_initializer=self.initialize_weights,
        ...
        model.add(
            Conv2D(filters=self.config["Layers_C"],
                   kernel_size=(self.config["Kernel_C"], self.config["Kernel_C"]),
                   kernel_initializer=self.initialize_weights,
                   bias_initializer=self.initialize_bias,
                   kernel_regularizer=l2(2e-4),
                   name='Conv3'
        ...
        model.add(
            Conv2D(filters=self.config["Layers_D"],
                   kernel_size=(self.config["Kernel_D"], self.config["Kernel_D"]),
                   kernel_initializer=self.initialize_weights,
                   bias_initializer=self.initialize_bias,
                   kernel_regularizer=l2(2e-4),
                   name='Conv4'
